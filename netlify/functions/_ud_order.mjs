// Shared UD order-schedule builder for the Bossa Sunningdale bar dashboard.
//
// Both order-xlsx.mjs (returns the .xlsx as a download) and send-order.mjs
// (attaches the .xlsx to a server-sent email) call buildOrderXlsx() so the two
// paths produce a byte-identical spreadsheet from the same committed template.
//
// United Distributors require their order as a filled-in copy of their own
// Excel catalogue, not a text email body. buildOrderXlsx loads the committed
...
// "Additional items" block so nothing is silently dropped.
//
...
// fallback, so it resolves across environments.
...
export function formatOrderDate(s) {
...
// Build the filled UD order spreadsheet from a validated payload.
//   payload { order_date, items: [{ key, qty }], extra: [{ name, unit, qty }] }
// Returns { buffer, filename }. Throws on template / write failure.
export async function buildOrderXlsx(payload) {
  ...
  if (!ws) throw new Error("template-empty");
