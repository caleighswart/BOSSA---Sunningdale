...
> **⏸️ PAUSED 2026-08-28 — project on hold.** Both workflows below still exist and
> still work, but their `schedule:` blocks are **commented out**, so **nothing runs
> automatically**: no daily dashboard refresh, no health check, no Netlify deploys.
> Trigger either manually from the Actions tab (`workflow_dispatch`) if you need a
> one-off refresh. **To resume:** uncomment the two `cron:` lines in
> `daily_bar.yml` and the two in `health_check.yml` — that is the whole change.
> Everything described below is what happens *when the schedules are restored*.
> See the 2026-08-28 row in the issues log for the full rationale and the Netlify
> billing caveat.
...
| 2026-08-28 | **Project put on hold (owner).** Dashboard no longer needed daily; owner moving to cancel Netlify hosting to stop ongoing cost. Left running, the four daily crons would keep pulling PilotLive and — once the dashboard stopped being refreshed or the site went away — `health_check.yml` would open a GitHub issue **and email a failure every single day**, repeating the issues #9–#21 nuisance from Jun 2026. | **Paused both active workflows.** `schedule:` blocks in `daily_bar.yml` and `health_check.yml` **commented out, not deleted**; `workflow_dispatch:` retained on both, so a manual run still regenerates the dashboard or runs a spot health check. Titles suffixed `(paused)` so the Actions tab reads honestly. **Zero active crons repo-wide** (the inventory/prep workflows were already dormant since 2026-05-13). No application code touched — generator, functions, pars and templates are all untouched. **Re-enable:** uncomment the two cron lines in each file; nothing else needed. **Billing note:** repo is public so GitHub Actions minutes were always free, and the Jun 2026 quota outage happened at the 300 build-min/month **Starter (free)** ceiling — strong evidence the Netlify plan was free all along, i.e. possibly nothing to cancel. Verify under Netlify → Team settings → Billing before changing plans. If downgrading, prefer **downgrade to Starter over deleting the site**: deletion drops the Netlify Blobs data (order confirmations + saved par overrides), breaks the `/confirm` links in already-sent supplier emails, and removes all five functions. **Outcome (same day): owner downgraded the Netlify plan; site kept, not deleted.** Verified live post-downgrade — `/` returns HTTP 200 (571 KB), and both `/api/order-status` and `/api/par-overrides` return 200 `{}`, so **Functions + Blobs still work on the free tier** and the `/confirm` links in already-sent supplier emails remain valid. Live `dashboard-generated` marker is frozen at `2026-08-27T18:32 SAST` (the last build before the pause) and **will stay frozen by design** — that is expected, not a fault, and nothing monitors it now that `health_check.yml` is paused too. |
