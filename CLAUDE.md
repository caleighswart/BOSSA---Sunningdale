...
| Netlify build hook | GitHub Secret | `NETLIFY_BUILD_HOOK_URL` (Netlify → Site settings → Build & deploy → Build hooks, branch `main`). POSTed by `daily_bar.yml` after each push and by `health_check.yml` on a stale-live recovery to force a deploy independent of Netlify's Git webhook. Optional — workflows fall back to Git auto-deploy if unset. |
